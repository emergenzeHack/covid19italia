...
data.append({ 'Tipo': "Accettate", 'Valore': repoSegnalazioni.get_issues(labels=["accettato"]).totalCount })

for label in labels:
	if label not in ["accettato"]:
		count = repoSegnalazioni.get_issues(labels=["accettato", label.name]).totalCount
		if count != 0:
